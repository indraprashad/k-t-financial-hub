...
import { fetchAllContent, fetchHomeContent, fetchAboutContent, fetchServicesContent, fetchBlogPosts, fetchBlogCategories, fetchBusinessContact, fetchConsultationContent, saveHomeContent, saveAboutContent, saveServiceContent, HomeContent, AboutContent, ServiceItem, BlogPost, BlogCategory, ContactContent, ConsultationContent, SiteContent } from './databaseStore';
...
const DEFAULT: SiteContent = {
  ...
  categories: [
    { id: "tax", name: "Tax", description: "Tax-related articles and updates" },
    { id: "accounting", name: "Accounting", description: "Accounting best practices and guides" },
    { id: "business", name: "Business", description: "Business strategy and management tips" }
  ],
  contact: {
    phone: "+975 17836510",
...
export function getContent(): SiteContent {
  try {
    ...
    if (raw) {
      ...
      const homeContent = { ...DEFAULT.home, ...parsed.home };
      
      // Handle CTA migration from object to array
      if (parsed.home?.cta && !Array.isArray(parsed.home.cta)) {
        // Old format: { title: string, subtitle: string }
        // New format: [{ title: string, subtitle: string }]
        homeContent.cta = [parsed.home.cta as any];
      } else {
        homeContent.cta = parsed.home?.cta ?? DEFAULT.home.cta;
      }
      
      return {
        home: { 
          ...homeContent, 
          hero: { ...DEFAULT.home.hero, ...parsed.home?.hero } 
        },
        about: { ...DEFAULT.about, ...parsed.about },
        services: parsed.services ?? DEFAULT.services,
        blog: parsed.blog ?? DEFAULT.blog,
        categories: parsed.categories ?? DEFAULT.categories,
...
const ADMIN_NAME = "Admin User"; // You can change this to the actual admin name
...
export async function syncWithSupabase() {
  try {
    ...
    const statsData = await fetchHomeContent('stats')
    if (statsData.length > 0) {
      const stats = statsData.map(item => ({ value: item.value || '', label: item.label || '' }))
      
      const current = getContent()
      const updated = { ...current, home: { ...current.home, stats } }
      saveContent(updated)
    }

    // Fetch home services from Supabase
    const homeServicesData = await fetchHomeContent('services')
    if (homeServicesData.length > 0) {
      const services = homeServicesData.map(item => ({ title: item.title || '', desc: item.description || '' }))
      
      const current = getContent()
      const updated = { ...current, home: { ...current.home, services } }
      saveContent(updated)
    }

    // Fetch why us points from Supabase
    const whyUsData = await fetchHomeContent('why_us_points')
    if (whyUsData.length > 0) {
      const points = whyUsData.map(item => ({ title: item.title || '', text: item.text || '' }))
      
      const current = getContent()
      const updated = { ...current, home: { ...current.home, whyUs: { ...current.home.whyUs, points } } }
      saveContent(updated)
    }

    // Fetch CTA banners from Supabase
    const ctaData = await fetchHomeContent('cta_banners')
    if (ctaData.length > 0) {
      const cta = ctaData.map(item => ({ title: item.title || '', subtitle: item.subtitle || '' }))
      
      const current = getContent()
      const updated = { ...current, home: { ...current.home, cta } }
      saveContent(updated)
    }

    // Fetch team members from Supabase
    const teamData = await fetchAboutContent('team_member')
    if (teamData.length > 0) {
      ...
      const current = getContent()
      const updated = { ...current, about: { ...current.about, team } }
      saveContent(updated)
    }

    // Fetch milestones from Supabase
    const milestonesData = await fetchAboutContent('milestone')
    if (milestonesData.length > 0) {
      const milestones = milestonesData.map(item => ({ 
        year: item.year || '', 
        text: item.text || '' 
      }))
      
      const current = getContent()
      const updated = { ...current, about: { ...current.about, milestones } }
      saveContent(updated)
    }

    // Fetch services from Supabase
    const fullServicesData = await fetchServicesContent()
    if (fullServicesData.length > 0) {
      const services = fullServicesData.map(item => ({ 
        id: item.service_id,
        title: item.title || '', 
...
export async function saveToSupabase(content: SiteContent) {
  try {
    // Save stats
    for (let i = 0; i < content.home.stats.length; i++) {
      const stat = content.home.stats[i]
    ...
    // Save home services
    for (let i = 0; i < content.home.services.length; i++) {
      const service = content.home.services[i]
      await saveHomeContent({
    ...
    // Save why us points
    for (let i = 0; i < content.home.whyUs.points.length; i++) {
      const point = content.home.whyUs.points[i]
      await saveHomeContent({
    ...
    // Save CTA banners
    for (let i = 0; i < content.home.cta.length; i++) {
      const cta = content.home.cta[i]
      await saveHomeContent({
    ...
    for (let i = 0; i < content.about.team.length; i++) {
      const member = content.about.team[i]
    ...
    // Save milestones
    for (let i = 0; i < content.about.milestones.length; i++) {
      const milestone = content.about.milestones[i]
      await saveAboutContent({
