...
import { blogCategoryApi } from '@/services/blog-services';
...
const BlogCategoryForm = () => {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            try {
                setLoading(true);
                const item = await blogCategoryApi.getById(id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && id) await blogCategoryApi.update(id, data); else await blogCategoryApi.create(data);
    ...
    return (
        <div className="h-full flex flex-col animate-in fade-in duration-500">
            <PageHeader title={isEditing ? 'Edit Category' : 'Add Category'} subtitle={isEditing ? 'Update category details' : 'Create new blog category'} buttonLabel="Back" buttonIcon={<ArrowLeft className="w-4 h-4" />} onButtonClick={() => navigate('/admin/category')} />
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 space-y-6 w-full">
                    <div className="space-y-6">
                        <div>
                            ...
                            <input type="text" value={form.name} onChange={e => update('name', e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all" placeholder="e.g., Tax Tips" required />
                        </div>
                        <div>
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-slate-200">
                        <Button type="button" variant="outline" onClick={() => navigate('/admin/category')}>Cancel</Button>
